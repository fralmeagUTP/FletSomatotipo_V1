...
def sample_valoracion_record(**overrides):
    values = {
        "id_Somatotipo": 1,
        ...
        "DIAMETRO_CODO": 68,
        "PorcGrasoJonson": None,
        "PesoGrasoJhonston": None,
        "PorcRasoYuasz": 18.0,
        "PesoRasoYuazs": 11.7,
        "PorcGrasoFaulker": 20.0,
        "PesoRasoFaulker": 13.0,
        "PesoOseo": 8.1,
        "PesoResidual": 12.8,
        "Mma": 29.8,
        "Endomorfismo": 3.1,
...
class PdfServiceTests(unittest.TestCase):
    ...
    def test_build_valoracion_pdf_returns_pdf_bytes(self):
        record = sample_valoracion_record()
        ...
        self.assertIn(b"todo principal: Yuhasz", pdf)
    ...
    def test_build_longitudinal_pdf_returns_pdf_with_somatocarta(self):
        ...
        self.assertIn(b"Grasa: Yuhasz", pdf)
    ...
    def test_individual_pdfs_change_with_athlete_identity_and_measurements(self):
        first_pdf = build_valoracion_pdf(sample_valoracion_record())
        second_pdf = build_valoracion_pdf(
            sample_valoracion_record(
                id_Somatotipo=2,
                NOMBRE_DEPORTISTA="Luis Rios",
                IDENTI_DEPORTISTA="456",
                PESO_kg=78,
                IMC=24.1,
                Endomorfismo=2.4,
                Mesomorfismo=5.1,
        ...
        self.assertIn(b"Ana Perez", first_pdf)
        self.assertIn(b"Luis Rios", second_pdf)
